...
function SearchByNameOrListButton({props, children, nameOrList, len, name, ID}) {
    ...
    const addOrRemoveFunc = () =>{
        ...
        if(nameOrList=="name"){
            ...
            props.navigation.navigate("SelectUserFromListScreen", {userInfo: props.route.params.userInfo});
        }else if(nameOrList=="confirm"){
            console.log("confirm");
            props.navigation.navigate("ManageSpeakerAndGroupsPermissionsScreen", {userID: ID, userInfo: props.route.params.userInfo});
        }
    }
